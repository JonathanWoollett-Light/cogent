...
pub mod core {
    ...
    use arrayfire::{
        and, cols, constant, device_mem_info, diag_extract, div, eq, exp, gt, imax, log, matmul,
        max, maxof, mul, pow, randu, sigmoid, sum, sum_all, sum_by_key, tanh, transpose, Array,
        Dim4, MatProp,
    };
        ...
        pub fn batch_size(&mut self, batch_size: Proportion) -> &mut Trainer<'a> {
            self.batch_size = match batch_size {
                Proportion::Percent(percent) => {
                    (self.training_data.len() as f32 * percent) as usize
                }
    ...
    impl DenseLayer {
        ...
        fn forepropagate(&self, a: &Array<f32>, ones: &Array<f32>) -> (Array<f32>, Array<f32>) {
            let weighted_inputs: Array<f32> =
                matmul(&self.weights, &a, MatProp::NONE, MatProp::NONE);
    ...
    impl NeuralNetwork {
        ...
        fn to_vec<T: arrayfire::HasAfEnum + Default + Clone>(
            array: &arrayfire::Array<T>,
        ) -> Vec<T> {
            ...
            for layer in self.layers.iter_mut() {
                activation = match layer {
                    ...
                    InnerLayer::Dense(dense_layer) => {
                        dense_layer.forepropagate(&activation, &ones).0
                    }
                };
            }
        ...
        pub fn train(&mut self, training_data: &[(Vec<f32>, usize)]) -> Trainer {
            ...
            for i in 0..training_data.len() {
                ...
                if training_data[i].0.len() != self.inputs as usize {
                    panic!(
                        "Input size of example {} ({}) != size of input layer ({}).",
                        i,
                        training_data[i].0.len(),
                        self.inputs
                    );
                }
            ...
            let batch_size: usize = if training_data.len() < 100usize {
                training_data.len()
            } else if batch_holder < 100f32 {
                100usize
            } else {
        ...
        fn train_details(
            ...
            let starting_evaluation =
                self.inner_evaluate(&matrix_evaluation_data, evaluation_data, cost);
            ...
            loop {
                ...
                if evaluation.1 as usize == evaluation_data.len() {
                    break;
                }
                ...
                match halt_condition {
                    ...
                    Some(HaltCondition::Accuracy(accuracy)) => {
                        if evaluation.1 >= (evaluation_data.len() as f32 * accuracy) as u32 {
                            break;
                        }
                    }
                    _ => {}
                ...
                match evaluation_min_change {
                    Proportion::Percent(percent) => {
                        if (evaluation.1 as f32 / evaluation_data.len() as f32)
                            > (best_accuracy as f32 / evaluation_data.len() as f32) + percent
                        {
        ...
        fn backpropagate(
            ...
            for (layer, (a, z)) in izip!(l_iter, out_iter) {
                ...
                partial_error = match layer {
                    InnerLayer::Dropout(dropout_layer) => {
                        dropout_layer.backpropagate(&partial_error)
                    }
        ...
        fn matrixify(&self, examples: &[(Vec<f32>, usize)]) -> (Array<f32>, Array<f32>) {
            ...
            let output: Array<f32> = Array::<f32>::new(
                &out_vec,
                Dim4::new(&[out_len as u64, example_len as u64, 1, 1]),
            );
            ...
            for layer in self.layers.iter() {
                layers.push(match layer {
                    ...
                    InnerLayer::Dense(dense_layer) => {
                        ...
                        let mut weight_holder =
                            vec![f32::default(); dense_layer.weights.elements()];
